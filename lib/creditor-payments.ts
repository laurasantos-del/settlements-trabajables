...
import { reconciliationCategory } from "@/lib/utils";
...
export function clientSummaries(payments: RawRecord[], interactions: RawRecord[]): RawRecord[] {
  ...
  return Array.from(groups.entries()).map(([clientId, items]) => {
    ...
    const risk = days > 30 ? "Critical" : days > 20 ? "High" : days > 10 ? "Medium" : "Low";
    return {
      Client_ID: clientId,
...
export function dashboardKpis(payments: RawRecord[], interactions: RawRecord[]): Kpi[] {
  ...
  return [
    ...
    { title: "Casos Urgentes", value: brokenSettlements(payments, interactions).filter((row) => Number(row.Days_Since_Y) > 30).length, tone: "danger" },
    { title: "Alertas Reconciliación", value: reconciliationRows(payments).filter((row) => Number(row.Days_Overdue) <= 30).length, tone: "warning" },
...
export function brokenSettlements(payments: RawRecord[], interactions: RawRecord[]): RawRecord[] {
  const summaries = clientSummaries(payments, interactions);
  return summaries
    .filter((row) => Number(row.Days_Since_Y) > 30)
    .map((row) => ({
      ...row,
      Last_Y: lastPaidDate(payments.filter((payment) => payment.Client_ID === row.Client_ID)) || "Nunca",
      Action: Number(row.Days_Since_Y) > 30 ? "Escalar y validar cancelación del settlement" : "Contactar cliente y confirmar próximo pago"
    }));
}
...
export function reconciliationRows(payments: RawRecord[]): RawRecord[] {
  ...
  return overdueUnpaid(payments)
    ...
    .map((row) => ({
      ...
      Category: reconciliationCategory(daysBetween(row.Due_Date)),
...
export function atRiskRows(payments: RawRecord[], interactions: RawRecord[]): RawRecord[] {
  return clientSummaries(payments, interactions)
    .filter((row) => Number(row.Days_Since_Y) > 10 && Number(row.Days_Since_Y) <= 30)
...
export function urgentRows(payments: RawRecord[], interactions: RawRecord[]): RawRecord[] {
  ...
  for (const row of [...brokenSettlements(payments, interactions), ...activeNegativeRows(payments, interactions), ...noFundsRows(payments, interactions)]) {
    ...
    const priority = Number(row.Days_Since_Y ?? 0) > 30 || Number(row.Balance ?? row.Escrow ?? 0) < 0 ? 3 : Number(row.Shortfall ?? 0) > 0 ? 2 : 1;
    if (!existing || priority > Number(existing.Priority)) {
      byClient.set(clientId, {
        ...row,
        Priority: priority,
        Severity: priority === 3 ? "CRÍTICO" : priority === 2 ? "URGENTE" : "ATENCIÓN",
        Reason: Number(row.Days_Since_Y ?? 0) > 30 ? "Más de 30 días sin pago" : Number(row.Balance ?? row.Escrow ?? 0) < 0 ? "Balance negativo" : "Fondos insuficientes",
...
export function alertRows(payments: RawRecord[], interactions: RawRecord[]): RawRecord[] {
  ...
  for (const row of brokenSettlements(payments, interactions)) {
    alerts.push({ Severity: Number(row.Days_Since_Y) > 30 ? "Critical" : "High", Client_ID: row.Client_ID, Current_Creditor: "", Description: `${row.Days_Since_Y} días sin Y`, Action: row.Action, Detected_Date: dateKey() });
  }
  ...
  for (const row of reconciliationRows(payments)) {
    ...
    const severity = category === "Broken Settlement" || category === "Posible Broken" ? "High" : category === "Riesgo Alto" || category === "Medio" ? "Medium" : "Low";
    alerts.push({ Severity: severity, Client_ID: row.Client_ID, Current_Creditor: row.Current_Creditor, Description: `${row.Problem} · ${category}`, Action: row.Action, Detected_Date: dateKey() });
  }
