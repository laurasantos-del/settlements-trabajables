...
export function CreditorPaymentsDashboard() {
  ...
  const today = (store.settlementPayments ?? []).filter((row) => isSameDay(row["Due Date"]));
  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard kpi={{ title: "Creditor Payments Today", value: today.length, subtitle: `${formatMoney(today.reduce((s, r) => s + Math.abs(parseMoney(r.Amount)), 0))} · ${showing}`, tone: "warning" }} />
