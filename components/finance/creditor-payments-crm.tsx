...
function useCreditorPaymentData(): CreditorPaymentData {
  ...
  const creditorStatus = useMemo(() => store.creditorStatus ?? [], [store.creditorStatus]);
  return {
    payments,
    interactions,
    creditorStatus,
...
export function CreditorPaymentsCreditors() {
  const data = useCreditorPaymentData();
  const activeRows = data.payments.filter((row) => isPaid(row) && daysBetween(row.Due_Date) >= 0 && daysBetween(row.Due_Date) <= 60);
  const activeByCreditor = Array.from(activeRows.reduce((map, row) => {
    const creditor = String(row.Current_Creditor || "Unknown");
    const current = map.get(creditor) ?? { Current_Creditor: creditor, Deals: 0, Amount_Total: 0 };
    current.Deals = Number(current.Deals ?? 0) + 1;
    current.Amount_Total = Number(current.Amount_Total ?? 0) + parseMoney(row.Amount);
    map.set(creditor, current);
    return map;
  }, new Map<string, RawRecord>()).values());
  const topActive = [...activeByCreditor].sort((a, b) => Number(b.Deals) - Number(a.Deals)).slice(0, 10);
  const bottomActive = [...activeByCreditor].sort((a, b) => Number(a.Deals) - Number(b.Deals)).slice(0, 10);
  const brokenByCreditor = Array.from(data.creditorStatus
    .filter((row) => row["Creditor Status"] === "S99_Broken Settlment")
    .reduce((map, row) => {
      const creditor = String(row["Creditor Name"] || "Unknown");
      const current = map.get(creditor) ?? { Current_Creditor: creditor, Broken: 0 };
      current.Broken = Number(current.Broken ?? 0) + 1;
      map.set(creditor, current);
      return map;
    }, new Map<string, RawRecord>()).values()).sort((a, b) => Number(b.Broken) - Number(a.Broken));
  const brokenMap = new Map(brokenByCreditor.map((row) => [String(row.Current_Creditor), Number(row.Broken)]));
  const rows = creditorRows(data.payments).map((row) => ({
    ...row,
    Broken: brokenMap.get(String(row.Current_Creditor)) ?? 0
  }));
  return (
    <div className="grid gap-6">
      <Header title="Acreedores" subtitle="Performance y exposición por acreedor." />
      <ApiState loading={data.loading} error={data.error} retry={data.refresh} />
      <DataTable title="Top acreedores por negocios activos" rows={topActive} pageSize={10} columns={[
        { key: "Current_Creditor", label: "Acreedor" },
        { key: "Deals", label: "# Negocios" },
        { key: "Amount_Total", label: "Monto total", render: (row) => formatMoney(row.Amount_Total) }
      ]} />
      <DataTable title="Acreedores con menos negocios" rows={bottomActive} pageSize={10} columns={[
        { key: "Current_Creditor", label: "Acreedor" },
        { key: "Deals", label: "# Negocios" },
        { key: "Amount_Total", label: "Monto total", render: (row) => formatMoney(row.Amount_Total) }
      ]} />
      <DataTable title="Acreedores con más broken settlements" rows={brokenByCreditor} pageSize={10} columns={[
        { key: "Current_Creditor", label: "Acreedor" },
        { key: "Broken", label: "# Broken" }
      ]} />
      <DataTable title="Tabla completa de acreedores" rows={rows} columns={[
        { key: "Current_Creditor", label: "Acreedor" },
        { key: "Clients", label: "# Clientes" },
        { key: "Completed", label: "Pagos Y" },
...
export function BrokenSettlementsPage() {
  const data = useCreditorPaymentData();
  const paymentByClient = paymentsByClient(data.payments);
  const byClient = new Map<string, RawRecord>();
  const statusRows = data.creditorStatus.filter((row) => row["Creditor Status"] === "S99_Broken Settlment");
  const overdueRows = data.payments.filter((row) => !isPaid(row) && daysBetween(row.Due_Date) > 30);

  function upsert(clientId: string, row: RawRecord, source: string) {
    const related = paymentByClient.get(clientId) ?? [];
    const riskAmount = related.reduce((sum, item) => sum + (parseMoney(item.Total_Debt) || parseMoney(item.Balance) || parseMoney(item.Settlement_Balance)), 0);
    const existing = byClient.get(clientId);
    byClient.set(clientId, {
      ...existing,
      ...row,
      Client_ID: clientId,
      Current_Creditor: String(row.Current_Creditor ?? row["Creditor Name"] ?? existing?.Current_Creditor ?? ""),
      Source: existing?.Source ? `${existing.Source} + ${source}` : source,
      Amount_Risk: existing?.Amount_Risk ?? riskAmount,
      Days_Since_Y: Math.max(Number(existing?.Days_Since_Y ?? 0), Number(row.Days_Since_Y ?? row.Days_Overdue ?? 0))
    });
  }

  for (const row of statusRows) upsert(String(row["Client ID"]), row, "Creditor Status");
  for (const row of overdueRows) upsert(String(row.Client_ID), { ...row, Days_Overdue: daysBetween(row.Due_Date) }, "Sin pago 30d+");
  const rows = Array.from(byClient.values()).sort((a, b) => Number(b.Days_Since_Y ?? 0) - Number(a.Days_Since_Y ?? 0));

  return (
    <div className="grid gap-6">
      <Header title="Broken Settlements" subtitle="Creditor Status S99 plus creditor payments without Y for more than 30 days." />
      <ApiState loading={data.loading} error={data.error} retry={data.refresh} />
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <KpiCard kpi={{ title: "Total Broken", value: rows.length, tone: "danger" }} />
        <KpiCard kpi={{ title: "De Creditor Status", value: new Set(statusRows.map((row) => String(row["Client ID"]))).size, tone: "danger" }} />
        <KpiCard kpi={{ title: "Sin pago 30d+", value: new Set(overdueRows.map((row) => String(row.Client_ID))).size, tone: "warning" }} />
        <KpiCard kpi={{ title: "Monto En Riesgo", value: rows.reduce((sum, row) => sum + parseMoney(row.Amount_Risk), 0), tone: "danger" }} />
      </section>
      <DataTable title="Broken Settlements" rows={rows} columns={[
      { key: "Client_ID", label: "Cliente" },
      { key: "Current_Creditor", label: "Acreedor" },
      { key: "Source", label: "Fuente", render: (row) => statusBadge(row.Source) },
      { key: "Days_Since_Y", label: "Días sin pago" },
      { key: "Amount_Risk", label: "Monto en riesgo", render: (row) => formatMoney(row.Amount_Risk) },
...
export function AtRiskPage() {
  ...
  return <RiskPage title="At Risk" subtitle="Casos con señales preventivas antes de convertirse en broken." rows={rows} loading={data.loading} error={data.error} retry={data.refresh} columns={[
    { key: "Client_ID", label: "Cliente" },
    { key: "Program", label: "Program" },
    { key: "Current_Creditor", label: "Acreedor" },
...
export function UrgentPage() {
  ...
  return <RiskPage title="Urgentes" subtitle="Broken 30d+, active negative y sin fondos críticos agrupados por prioridad." rows={rows} loading={data.loading} error={data.error} retry={data.refresh} columns={[
    { key: "Client_ID", label: "Cliente" },
    { key: "Program", label: "Program" },
...
export function ChatPage() {
  ...
  function answer(question: string) {
    ...
    if (lower.includes("riesgo")) return `${atRiskRows(data.payments, data.interactions).length} clientes están en riesgo preventivo y ${brokenSettlements(data.payments, data.interactions).length} están broken.`;
