...
export function FinanceOverview() {
  ...
  const activeClients = useMemo(
    () => interactions.filter((r) => clientStatus(r) === "Active"),
    [interactions]
  );
  const nsfClients = useMemo(
    () => interactions.filter((r) => ["NSF", "NSF - First Payment"].includes(clientStatus(r))),
    [interactions]
  );
  const onHoldClients = useMemo(
    () => interactions.filter((r) => clientStatus(r) === "On Hold"),
    [interactions]
  );
  const waitingClients = useMemo(
    () => interactions.filter((r) => clientStatus(r) === "Waiting for first payment"),
    [interactions]
  );
  ...
  const feesCobrados = useMemo(
    () => paymentsCleared.reduce((s, r) => s + parseMoney(r["Service Fee Amount"] ?? r["Service Fee"]), 0),
    [paymentsCleared]
  );
  ...
  const serviceFeeCols: ColDef[] = [
    { key: "Client ID", label: "Cliente" },
    { key: "Date Cleared", label: "Fecha" },
    {
      key: "Service Fee Amount",
      label: "Service Fee",
      render: (r) => formatMoney(r["Service Fee Amount"] ?? r["Service Fee"]),
    },
  ];
  ...
  const revenueSection: CardDef[] = [
    ...
    },
    {
      id: "fees-cleared",
      label: "Fees Cobrados",
      value: formatMoney(feesCobrados),
      borderColor: "#22c55e",
      rows: paymentsCleared.filter((r) => parseMoney(r["Service Fee Amount"] ?? r["Service Fee"]) > 0),
      columns: serviceFeeCols,
      allHref: "/finance/revenue",
    },
    {
      id: "cleared",
  ...
  const alerts = useMemo((): AlertRow[] => {
    return settlementPayments
      ...
      .map((r) => {
        const d = daysBetween(r["Due Date"]);
        const cat = reconciliationCategory(d);
  ...
  const seg = (cat: string): RawRecord[] => alerts.filter((r) => r._category === cat);
  ...
  const alertsSection: CardDef[] = [
    {
      ...
      borderColor: "#3b82f6",
      badge: { text: "1–10 días", color: "#3b82f6", bg: "#3b82f620" },
    ...
    },
    {
      id: "seg2",
      label: "Medio",
      value: seg("Medio").length,
      borderColor: "#eab308",
      badge: { text: "10–15 días", color: "#eab308", bg: "#eab30820" },
      rows: seg("Medio"),
      columns: alertCols,
      allHref: "/finance/alertas",
    },
    {
      ...
      borderColor: "#f97316",
      badge: { text: "15–20 días", color: "#f97316", bg: "#f9731620" },
    ...
    },
    {
      ...
      borderColor: "#ef4444",
      badge: { text: "20–30 días", color: "#ef4444", bg: "#ef444420" },
    ...
    },
    {
      ...
      borderColor: "#7f1d1d",
      badge: { text: "+30 días", color: "#fca5a5", bg: "#7f1d1d40" },
